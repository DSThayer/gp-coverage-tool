--Test 9
--When max_gap_to_fill is set to 100, a gap of 100 days between registrations is closed.

--2) Run the procedure

call sail0286v.clean_gp_regs(
	--Use a unique table name so it doesn't clash with anyone else testing.
	target_table=>'sailx286v.test9',
...
declare global temporary table session.gptest9table
(aPERS_ID_E integer, 
aFROM_DT date,
aTO_DT date,
bPERS_ID_E integer, 
...
Insert Into session.gptest9table
select a.PERS_ID_E, 
a.FROM_DT,
a.TO_DT,
b.PERS_ID_E, 
...
where a.from_dt = b.to_dt + 100 days
...
insert into sailx286v.test_results
	with test_query as (
		--Test query here.
			...
			select count(*) as num_gap_100_days from session.gptest9table t
			join sailx286v.test9 t9
			on t.alf_e = t9.alf_e and t.afrom_dt = t9.start_date
	)
	select 
		--Enter the number of the test according to the spreadsheet here.
		9 as test_num,
		'When max_gap_to_fill is set to 100, a gap of 100 days between registrations is closed.' as description,
		--test condition that proves whether the test passed here
		case when num_gap_100_days = 0 	
			then 'pass' else 'fail'
		end as result,
...
drop table sailx286v.test9!
drop table session.gptest9table!
