--Test 23
--If group_on_sail_data is 1, a GP registration that overlaps the beginning and end of a good data period is broken into three registrations: one with GP data and two without GP data (set keep_non_sail_recs to 1 to test).

--2) Run the procedure
call fnc.drop_if_exists ('sailx0286v.test23')

call sailx0286v.clean_gp_regs(
	--Use a unique table name so it doesn't clash with anyone else testing.
	target_table=>'sailx0286v.test23',
...
insert into sailx0286v.test_results
	with test_query as (
		--Test query here.
		select count(*) as num_split_gp_reg from (
		select count(*), alf_e from sailx0286v.test23
			where alf_e in (		
	...
	select 
		...
		23 as test_num,
		'If group_on_sail_data is 1, a GP registration that overlaps the beginning and end of a good data period is broken into three registrations: one with GP data and two without GP data (set keep_non_sail_recs to 1 to test).' as description,
		--test condition that proves whether the test passed here
		case when num_split_gp_reg = 0
			then 'pass' else 'fail'
		end as result,
