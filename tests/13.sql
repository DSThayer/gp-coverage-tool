--Test 13
--When threshold is set to .25, a month with rate just under .25 is not counted as good.

--2) Run the procedure

call sail0286v.clean_gp_regs(
	--Use a unique table name so it doesn't clash with anyone else testing.
	target_table=>'sailx286v.test13',
...
declare global temporary table session.gptest13table
(prac_cd_e integer, 
data_start date,
data_end date,
...
insert into session.gptest13table
select prac_cd_e, 
...
from (
	select prac_cd_e, 
			...
			(row_number() over 
				( 	partition by prac_cd_e, 
					case when relative_event_rate < .25 then 0 
					else 1 
					end 
					order by prac_cd_e, event_yr, event_mo) 
			) as data_group, 
...
insert into sailx286v.test_results
	...
	select 
		...
		13 as test_num,
		'When threshold is set to .25, a month with rate just under .25 is not counted as good.' as description,
