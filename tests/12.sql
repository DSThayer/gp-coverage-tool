--Test 12
--When threshold is set to 1,  longest continuous period of months with event_rate >= 1 are counted as having GP data.

--2) Run the procedure

call sail0286v.clean_gp_regs(
	--Use a unique table name so it doesn't clash with anyone else testing.
	target_table=>'sailx286v.test12',
...
--create a temp table to hold identified registrations with threshold of >=1


declare global temporary table session.gptest12table
(prac_cd_e integer, 
data_start date,
data_end date,
...
insert into session.gptest12table
select prac_cd_e, 
...
from (
	select prac_cd_e, 
			...
			(row_number() over 
				( 	partition by prac_cd_e, 
					case when relative_event_rate >= 1 then 1 
					else 0 
					end 
					order by prac_cd_e, event_yr, event_mo) 
			) as data_group, 
...
insert into sailx286v.test_results
	...
	select 
		...
		12 as test_num,
		'When threshold is set to 1,  longest continuous period of months with event_rate >= 1 are counted as having GP data.' as description,
